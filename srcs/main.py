...
for count, line in enumerate(file, start=0):
    ...
    store.add((hospital, GEO["lat"], Literal(line[14])))
    store.add((hospital, GEO["long"], Literal(line[15])))
    store.add((hospital, FOAF.phone, Literal(line[18])))
    store.add((hospital, FOAF.email, Literal(line[19])))
    store.add((hospital, FOAF.homepage, Literal(line[20])))
